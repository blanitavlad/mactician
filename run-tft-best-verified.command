...
if (( print_config == 1 )); then
    env | LC_ALL=C sort | grep -E \
        '^(TFT_(ADB_SERVER_PORT|AUDIO_ENABLED|AVD_NAME|CPU_CORES|DISPLAY_DENSITY|DISPLAY_SIZE|EMULATOR|GLTRANSPORT|GRAPHICS_PROFILE|GUEST_GL_DRIVER|GUEST_SUBMIT_THREAD|HWUI_RENDERER|INPUT_BRIDGE_ENABLED|LAUNCHER|MACOS_GAME_MODE|MEMORY_MB|MVK_QUEUE_MODE|RENDERER|SERIAL|UI_SCALE|VIRTIO_GPU_NATIVE_SYNC|VIRTIO_GPU_NEXT|VULKAN_BATCHED_DESCRIPTORS)=|MVK_CONFIG_(FAST_MATH_ENABLED|MAX_ACTIVE_METAL_COMMAND_BUFFERS_PER_QUEUE)=)'
    exit 0
fi
