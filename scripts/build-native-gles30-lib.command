...
readonly OUTPUT_DIR="$PROJECT_DIR/artifacts/tft-pbe-18.1-5212127-native-gles30"
...
readonly PATCHED_SHA="5500b8cd407161b62c1dcf55c4ac47e75ac4b32cc88f735578f8c0e12e30d437"
...
if [[ -f "$OUTPUT_LIB" ]]; then
    if [[ "$(shasum -a 256 "$OUTPUT_LIB" | awk '{ print $1 }')" == "$PATCHED_SHA" ]] \
            && [[ "$(xxd -p -l 4 -s "$PATCH_OFFSET" "$OUTPUT_LIB")" == "1f000071" ]]; then
        print "The verified native-GLES 3.0 libUnreal.so is already available: $OUTPUT_LIB"
        exit 0
    fi
...
/usr/bin/printf '\037\000\000\161' \
    | /bin/dd of="$NEXT_LIB" bs=1 seek="$PATCH_OFFSET" conv=notrunc status=none
if [[ "$(xxd -p -l 4 -s "$PATCH_OFFSET" "$NEXT_LIB")" != "1f000071" ]] \
        || [[ "$(shasum -a 256 "$NEXT_LIB" | awk '{ print $1 }')" != "$PATCHED_SHA" ]]; then
    print "The ES 3.2→3.0 gate patch did not produce the expected binary; no output is published."
    exit 1
fi
/bin/mv "$NEXT_LIB" "$OUTPUT_LIB"

print "Temporary libUnreal.so ES 3.0 gate patch is ready: $OUTPUT_LIB"
print "One four-byte ARM64 instruction at 0x3c72f48: cmp minor,#2 → cmp minor,#0."
